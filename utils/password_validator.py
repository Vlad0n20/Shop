...
class PasswordCharacterValidator:
    ...
    def validate(self, password, user=None):
        ...
        if (
            len([char for char in password if char in self.special_characters])
            < self.min_length_special
        ):
            validation_errors.append(
                ValidationError(
                    ungettext(
                        "This password must contain at least %(min_length)d special "
                        "character such as %(special_characters)s.",
                        "This password must contain at least %(min_length)d special characters "
                        "such as %(special_characters)s.",
                        self.min_length_special,
                    ),
                    params={
    ...
    def get_help_text(self):
        ...
        if self.special_characters:
            validation_req.append(
                ...
                % {
                    "min_length_special": str(self.min_length_special),
                    "special_characters": self.special_characters,
                }
            )
        return (
